...
public class SubscriptionService {
	...
	// method that updates a subscription
	public SubscriptionResponseDTO updateSubscription(Long studentId, Long planId) {

		Subscription subscription = subscriptionRepository.findById(studentId).orElseThrow(
				() -> new ResourceNotFoundException("Subscription not found"));

		if (subscription.getPlan().getId().compareTo(planId) == 0)
			throw new ResourceDuplicateException("Subscription already exists to this plan");
		...
		LocalDate beginDate = LocalDate.now();
		LocalDate endDate = beginDate.plusMonths(plan.getType() == "Trimestral" ? 3 : 1);
